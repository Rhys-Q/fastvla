...
from fastvla.configs.policies import PreTrainedConfig
from fastvla.configs.types import FeatureType, PolicyFeature
from fastvla.datasets.utils import build_dataset_frame
from fastvla.processor import PolicyAction, RobotAction, RobotObservation
from fastvla.utils.constants import ACTION, OBS_STR
...
def make_robot_action(action_tensor: PolicyAction, ds_features: dict[str, dict]) -> RobotAction:
    ...
    act_processed_policy: RobotAction = {f"{name}": float(action_tensor[i]) for i, name in enumerate(action_names)}
