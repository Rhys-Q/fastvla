...
factory functions from `fastvla.policies.factory` to create processors and configurations,
...
from fastvla.configs.types import FeatureType, NormalizationMode, PolicyFeature
from fastvla.policies.factory import get_policy_class, make_policy_config, make_pre_post_processors
        ...
        for feature_type_str, mode_str in config["normalization_mapping"].items():
            ...
            try:
                if mode_str == "MEAN_STD":
                    ...
                    print(f"Warning: Unknown normalization mode '{mode_str}' for feature type '{feature_type_str}'")
                    continue
            except (AttributeError, ValueError):
...
def clean_state_dict(state_dict: dict[str, torch.Tensor], remove_str: str = "._orig_mod") -> dict[str, torch.Tensor]:
    """
...
def display_migration_summary_with_warnings(problematic_missing_keys: list[str]) -> None:
    ...
    print("=" * 60)
    print(f"The migration was successful, but {len(problematic_missing_keys)} unexpected missing keys were found:")
    print()
    for key in problematic_missing_keys:
        print(f"   - {key}")
    print()
...
from fastvla.policies import get_policy_class
from fastvla.processor import PolicyProcessorPipeline
...
*Generated automatically by the LeRobot policy migration script*"""
    ...
    if args.push_to_hub and hub_repo_id:
        ...
        if args.branch:
            ...
            print(f"View the PR at: https://huggingface.co/{hub_repo_id}/discussions (look for the most recent PR)")
        else:
