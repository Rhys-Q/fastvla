...
from fastvla.configs.types import FeatureType, NormalizationMode, PipelineFeatureType, PolicyFeature
from fastvla.datasets.lerobot_dataset import LeRobotDataset
from fastvla.utils.constants import ACTION
...
class _NormalizationMixin:
    ...
    **Stats Override Preservation:**
    ...
    Examples:
        ...
        from fastvla.datasets import LeRobotDataset
        ...
        if self.features:
            ...
            if isinstance(first_val, dict):
                ...
                for key, ft_dict in self.features.items():
                    reconstructed[key] = PolicyFeature(type=FeatureType(ft_dict["type"]), shape=tuple(ft_dict["shape"]))
    ...
    def to(self, device: torch.device | str | None = None, dtype: torch.dtype | None = None) -> _NormalizationMixin:
        """
    ...
    def load_state_dict(self, state: dict[str, Tensor]) -> None:
        ...
        **Stats Override Preservation:**
        ...
        for flat_key, tensor in state.items():
            ...
            self._tensor_stats.setdefault(key, {})[stat_name] = tensor.to(dtype=torch.float32, device=self.device)
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        config = {
            "eps": self.eps,
            "features": {key: {"type": ft.type.value, "shape": ft.shape} for key, ft in self.features.items()},
    ...
    def _apply_transform(self, tensor: Tensor, key: str, feature_type: FeatureType, *, inverse: bool = False) -> Tensor:
        """
...
class NormalizerProcessorStep(_NormalizationMixin, ProcessorStep):
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        if observation is not None:
            new_transition[TransitionKey.OBSERVATION] = self._normalize_observation(observation, inverse=False)
