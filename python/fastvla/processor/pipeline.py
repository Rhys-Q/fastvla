...
from fastvla.configs.types import PipelineFeatureType, PolicyFeature
from fastvla.utils.hub import HubMixin
...
class DataProcessorPipeline(HubMixin, Generic[TInput, TOutput]):
    ...
    def _save_pretrained(self, save_directory: Path, **kwargs):
        ...
        for step_index, processor_step in enumerate(self.steps):
            ...
            if registry_name:
                ...
                step_entry["class"] = f"{processor_step.__class__.__module__}.{processor_step.__class__.__name__}"
    ...
    def save_pretrained(
        ...
        **push_to_hub_kwargs,
        ...
        if save_directory is None:
            ...
            from fastvla.utils.constants import HF_LEROBOT_HOME
        ...
        **Benefits of Explicit config_filename**:
        ...
        if model_path.is_dir():
            ...
            if not config_path.exists():
                ...
                raise FileNotFoundError(f"Config file '{config_filename}' not found in directory '{model_id}'")
    ...
    @classmethod
    def _validate_loaded_config(cls, model_id: str, loaded_config: dict[str, Any], config_filename: str) -> None:
        ...
        **For each step in loaded_config["steps"]**:
        ...
        1. **Class Resolution** (via _resolve_step_class):
           ...
           - **Else use "class" field**: Dynamic import from full module path
             Example: {"class": "fastvla.processor.normalize.NormalizeStep"}
    ...
    def _resolve_step_class(cls, step_entry: dict[str, Any]) -> tuple[type[ProcessorStep], str]:
        ...
        **Tier 2: Dynamic import fallback**:
        - **Else use "class" field**: Full module.ClassName import path
          ...
          - **Example**: "fastvla.processor.normalize.NormalizeStep"
            a. Import module: "fastvla.processor.normalize"
    ...
    @classmethod
    def _validate_overrides_used(cls, remaining_override_keys: set[str], loaded_config: dict[str, Any]) -> None:
        ...
        **Helpful Error Generation**:
        - **Extract available keys**: Build list of valid override keys from config
          ...
          - Example: "fastvla.processor.normalize.NormalizeStep" -> "NormalizeStep"
    ...
    def process_action(self, action: PolicyAction | RobotAction | EnvAction) -> PolicyAction | RobotAction | EnvAction:
...
class ActionProcessorStep(ProcessorStep, ABC):
    ...
    @abstractmethod
    def action(self, action: PolicyAction | RobotAction | EnvAction) -> PolicyAction | RobotAction | EnvAction:
